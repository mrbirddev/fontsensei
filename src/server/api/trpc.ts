...
export function getLocaleFromURL(url: string | undefined) {
  ...
  const narrowed = narrowLocaleString(slugLocale) ?? fallbackLocale;
  invariant(narrowed);
  return narrowed;
}
...
export const createTRPCContext = async (opts: CreateNextContextOptions) => {
  ...
  // Migration to next-auth v5 stuck here
  // https://github.com/nextauthjs/next-auth/issues/9307#issuecomment-2185667866
...
const t = initTRPC.context<typeof createTRPCContext>().create({
  ...
  errorFormatter({ shape, error }) {
    ...
    return {
      ...
      data: {
        ...shape.data,
        causeCode: error.cause instanceof Error ? error.cause.message : null,
        zodError:
