/*
# To run this script
  npx tsx src/@fontsensei/data/merge.ts

  Writes JSON under public/data/<unix_ms>/ and updates
  src/@fontsensei/data/generated/fontDataFolder.ts for the app to import.
...
const writeFontDataFolderModule = (folder: string) => {
  ...
  fs.writeFileSync(
    generatedPath,
    `/** Generated by merge.ts — do not edit by hand. */\nexport const FONT_DATA_FOLDER = '${folder}' as const;\n`,
    'utf-8'
  );
};
...
const mergeData = async () => {
  try {
    const dataFolder = dataFolderStamp();
    const outputDir = path.join(__dirname, '../../../public/data', dataFolder);
    fs.mkdirSync(outputDir, { recursive: true });
    const outputDirWithSlash = outputDir + path.sep;
    ...
    writeFontDataFolderModule(dataFolder);
    console.log('Data successfully merged and written!');
    console.log('Output directory:', path.relative(path.join(__dirname, '../../..'), outputDir));
